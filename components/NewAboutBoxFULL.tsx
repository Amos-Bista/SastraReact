...
const NewAboutBoxFull: React.FC<ReusableComponentProps> = ({ desc, limit }) => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (count < limit) {
        setCount((prevCount) => prevCount + 5); // Increase count rapidly initially
        if (intervalDuration < 40) {
          setIntervalDuration(
            (prevIntervalDuration) => prevIntervalDuration + 2
  ...
  return (
    <div className="h-68 w-80 border-b-4 border-sky-900 py-8 px-6 shadow-lg hover:shadow-2xl my-10 transition-all ease-in-out">
      <div className="flex justify-center flex-col">
        <h1 className="text-6xl text-sky-900 font-semibold mx-auto">
          {count}+
        </h1>
...
export default NewAboutBoxFull;
